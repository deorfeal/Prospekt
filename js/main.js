  ...
  if (document.querySelector(".preloader")) {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        // ← запускаем после preloader, вместе с AOS
        StrokeText.init();
    ...
    ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        if (self.progress >= 0.5) {
          ...
          footer.style.setProperty("--overlay-progress", p);
          gsap.to(footer, {
            backgroundColor: `rgba(6, 6, 6, ${p})`,
            duration: 0,
            overwrite: true,
          });
          gsap.to(footer.querySelectorAll(".footer-box__title, .footer-box__text"), {
            color: `rgba(250,250,250,${Math.min(p * 2, 1)})`,
            duration: 0,
            overwrite: true,
          });
        } else {
